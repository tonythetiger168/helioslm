...
class PretrainingTrainer:
    ...
    def train(self, dataloader, num_steps):
        ...
        for step in range(num_steps):
            ...
            input_ids = batch['input_ids'][:, :self._get_context_length(step, num_steps)].to(self.device)
            labels = batch['labels'][:, :self._get_context_length(step, num_steps)].to(self.device)
    ...
    def _get_context_length(self, step, num_steps):
        if step < num_steps * 0.3: return 32768
        elif step < num_steps * 0.6: return 131072
        return 1048576
