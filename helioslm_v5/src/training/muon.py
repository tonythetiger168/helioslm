...
Simplifications (documented, not hidden):
  - Per-MATRIX orthogonalization, not per-attention-HEAD. Splitting an
    attention projection into per-head blocks before orthogonalization is
    K3's "per-head" extension; it composes with any head layout via param
    groups and is left to the caller.
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.02, momentum=0.95, nesterov=True,
                 ...
                 betas=(0.9, 0.95), eps=1e-8, muon=True):
        if lr < 0.0:
