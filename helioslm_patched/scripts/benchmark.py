...
def benchmark(model, config, device, num_tokens=100):
    ...
    # Warmup
    with torch.no_grad():
        _ = model.generate(input_ids, max_new_tokens=10, use_speculative=False, temperature=1.0)
    ...
    t0 = time.time()
    with torch.no_grad():
        _ = model.generate(input_ids, max_new_tokens=num_tokens, use_speculative=False, temperature=1.0)
    if device.type == "cuda":
        torch.cuda.synchronize()
    ...
    if model.speculative_decoder is not None:
        ...
        t0 = time.time()
        with torch.no_grad():
            _ = model.generate(input_ids, max_new_tokens=num_tokens, use_speculative=True, temperature=1.0)
        if device.type == "cuda":
            torch.cuda.synchronize()
...
def main():
    ...
    for name, cfg in SIZE_CONFIGS.items():
        ...
        if spec > 0:
            print(f"   Speculative: {spec:.1f} tok/s | Speedup: {spec/std:.2f}x")
